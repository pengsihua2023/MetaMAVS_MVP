...
```bash
pip install -e ".[llm]"                           # 把 anthropic + python-dotenv 装进“运行 metamavs 的那个”环境
echo 'ANTHROPIC_API_KEY=sk-ant-...' > .env        # .env 已被 gitignore
```

> ⚠️ 必须装进 `metamavs` 实际使用的解释器(如 `.venv`)。若 `.venv` 缺 `anthropic`,
> 即使有 key,LLM 也会静默退回确定性逻辑(报告里会出现“no ANTHROPIC_API_KEY/SDK”)。
> 自检:`.venv/bin/python -c "from metamavs.llm.client import llm_available; print(llm_available())"` 应为 `True`。
...
**分 agent 设模型(可选)**:6 个 agent 默认共用上面的 `model`/`effort`/`max_tokens`;
在 `llm.overrides` 下按 agent 名(`qc`、`taxonomy`、`abundance`、`novel_virus`、
`risk_assessment`、`llm_interpretation`)单独覆盖,未写的继承默认:

```yaml
llm:
  enabled: true
  model: claude-opus-4-8       # 所有 agent 的默认
  effort: medium
  overrides:
    risk_assessment: { model: claude-opus-4-8, effort: high }   # 风险用强模型
    qc:              { model: claude-haiku-4-5, effort: low }    # QC 用便宜模型省钱
```
