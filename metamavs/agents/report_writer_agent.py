...
def _build_markdown(state: MetaMAVSState) -> str:
    ...
    if llm.get("status") == "ok" and llm.get("text"):
        parts.append(md_section("AI-Assisted Interpretation"))
        parts.append(
            f"_Generated by {llm.get('model', 'Claude')}. Advisory only — the deterministic "
            "risk assessment above is authoritative; confirm all high-risk signals._\n"
        )
